...
class Goldfisher:
    ...
    def reset(self):
        ...
        self.turn = 0
        self.draws = 0
        self.hare_apparents_drawn = 0  # Track Hare Apparent draws
        # reset zones
        self.command_zone = []
        self.deck = []
        self.yard = []
    ...
    def draw(self):
        ...
        self.draws += 1
        if drawn.name == "Hare Apparent":
            self.hare_apparents_drawn += 1
    ...
    def simulate(self):
        ...
        for j in tqdm(range(self.sims), leave=False):
            ...
            # Track Hare Apparent draws
            hare_drawn_list.append(self.hare_apparents_drawn)
            ...
            if j > sample_games:
                if top_centile_threshold is None:
                    ...
                    # Hare Apparent thresholds
                    hare_thresholds = {
                        'top_centile': np.percentile(hare_drawn_list, 99),
                        'low_centile': np.percentile(hare_drawn_list, 1),
                        'top_decile': np.percentile(hare_drawn_list, 90),
                        'low_decile': np.percentile(hare_drawn_list, 10),
                        'top_quartile': np.percentile(hare_drawn_list, 75),
                        'low_quartile': np.percentile(hare_drawn_list, 25),
                        'median': np.percentile(hare_drawn_list, 50),
                    }
                else:
                    # Record games in all applicable mana categories
                    record_games = []
                    ...
                    # Record games in Hare Apparent categories
                    hare_categories = []
                    
                    # Top categories (inclusive)
                    if self.hare_apparents_drawn >= hare_thresholds['top_centile']:
                        hare_categories.extend(["top_centile", "top_decile", "top_quartile", "top_half"])
                    elif self.hare_apparents_drawn >= hare_thresholds['top_decile']:
                        hare_categories.extend(["top_decile", "top_quartile", "top_half"])
                    elif self.hare_apparents_drawn >= hare_thresholds['top_quartile']:
                        hare_categories.extend(["top_quartile", "top_half"])
                    elif self.hare_apparents_drawn >= hare_thresholds['median']:
                        hare_categories.append("top_half")
                    
                    # Bottom categories (inclusive)
                    if self.hare_apparents_drawn <= hare_thresholds['low_centile']:
                        hare_categories.extend(["low_centile", "low_decile", "low_quartile", "low_half"])
                    elif self.hare_apparents_drawn <= hare_thresholds['low_decile']:
                        hare_categories.extend(["low_decile", "low_quartile", "low_half"])
                    elif self.hare_apparents_drawn <= hare_thresholds['low_quartile']:
                        hare_categories.extend(["low_quartile", "low_half"])
                    elif self.hare_apparents_drawn < hare_thresholds['median']:
                        hare_categories.append("low_half")
                    # Record mana stats
                    ...
                    # Record Hare Apparent stats
                    for hare_category in hare_categories:
                        hare_records[hare_category]["count"].append(self.hare_apparents_drawn)
        ...
        return (
            ...
            threshold_percent, threshold_mana, con_threshold, distribution_stats, hare_records
        )
...
def main(config):
    ...
    for i in tqdm(range(min_lands, max_lands), total=max_lands-min_lands):
        ...
        # Unpack the outcome tuple
        (land_count, mean_mana, consistency, mean_bad_turns, mean_mid_turns, 
         mean_lands, mean_mulls, mean_draws, percentile_25, percentile_50, percentile_75, 
         threshold_percent, threshold_mana, con_threshold, dist_stats, hare_records) = outcome
        
        # Store the main outcomes
        outcomes.append([
            land_count, mean_mana, consistency, mean_bad_turns, mean_mid_turns,
            mean_lands, mean_mulls, mean_draws, percentile_25, percentile_50, percentile_75,
        ...
        # Store Hare Apparent records
        hare_records_by_lands[i] = hare_records
    ...
    con_threshold = con_threshold*100
    print(f"\n-----------------------------------")
    ...
    # Calculate average Hare Apparents drawn for each category
    hare_outcomes = []
    for i in range(min_lands, max_lands):
        hare_stats = []
        for category in ["top_centile", "top_decile", "top_quartile", "top_half", 
                        "low_half", "low_quartile", "low_decile", "low_centile"]:
            counts = hare_records_by_lands[i][category]["count"]
            avg_hares = np.mean(counts) if counts else 0
            hare_stats.append(f"{avg_hares:.2f}")
        hare_outcomes.append([i] + hare_stats)

    print("\nHare Apparent Distribution Statistics (Average Drawn):")
    print("--------------------------------------------------")
    print(tabulate(hare_outcomes, headers=[
        "Land Ct",
        "Top 1%",
        "Top 10%",
        "Top 25%",
